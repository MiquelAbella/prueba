...
function openCard(event) {
  ...
  if (rightPart.classList.contains("content-individual-document")) {
    rightPart.classList.replace(
      "content-individual-document",
      "content-individual-document-show"
    );
  }
...
function subFolder(e) {
  ...
  if (createSubfolder.classList.contains("modal2-subFolder")) {
    createSubfolder.classList.replace(
      "modal2-subFolder",
      "modal2-show-subFolder"
    );
  }
}
...
function createNewSubFolder(e) {
  ...
  fetch(
    `./create-subfolder.php?foldername=${subFolderNewName}&path=${subFolderName}`,
    {
      method: "GET",
    }
  )
    ...
    .then((data) => {
      console.log(data);
      let parentElement = document.getElementById(data.parentDir);

      let path = `${data.parentDir}/${data.newDir}`;

      let childrenFolder = document.createElement("div");

      childrenFolder.classList.add("div-type-documents");
      childrenFolder.setAttribute("id", path);

      childrenFolder.innerHTML = `
      ...
      parentElement.insertAdjacentElement("afterend", childrenFolder);
